...
the regular expressions against the model path. The first matching regular
...
| ignore_git_paths | true or false      | Optional  | Ignore git related files such as `.git`, `.gitattributes` in the model path |
| use_staging      | true or false      | Optional  | The staging servers were used for signing with the 'sigstore' method |
| ignore_path      | list of file paths | Optional  | Files to ignore when verifying the signature |
...
### Signature Verification Quickstart Example

The following example clones a known signed AI model and enforces that it
was signed by a given expected signer.
...
cat <<_EOF_ >policy.json
{
  "policy": {
    "signatures": {
      ...
      "models": {
        "/tmp/test/granite-4.0-tiny-preview/": {
          "signer": "signer1"
        }
      }
    }
...
vllm serve --security-policy /tmp/test/policy.json /tmp/test/granite-4.0-tiny-preview
```

vLLM shows the following line in its log:

```text
(APIServer pid=143128) INFO 09-23 14:54:31 [security_policy.py:339] Signature verification succeeded on /tmp/test/granite-4.0-tiny-preview
```
